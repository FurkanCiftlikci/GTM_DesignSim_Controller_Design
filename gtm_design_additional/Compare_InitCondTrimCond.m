...
% Variables to plot
eas=zeros(length(Gamma),1);
elevator=zeros(length(Gamma),1);
throttle=zeros(length(Gamma),1);
fprintf('  Trimming at gamma:\n');
...
for trimpt=[1:length(Gamma)],
  ...
  loadmws(MWS,'gtm_design');
  [MWS,Xt,Tc,Err]=trimgtm(struct('gamma',Gamma(trimpt)), 'elev', 0);
  if (Err>1e-3),  % try again, different starting poing
      loadmws(init_design(),'gtm_design');
      fprintf('   Poor convergence, trying again. ');
      [MWS,Xt,Tc,Err]=trimgtm(struct('gamma',Gamma(trimpt)), 'elev', 0);
...
subplot(122),
[ax,h1,h2]=plotyy(Gamma,elevator,Gamma,[eas,throttle]); grid on
set(ax,{'YColor'},{[0 0 1];[1 0 0]});
set([h1;h2],{'Color'},{'blue';'red';'red'});
set([h1;h2],{'Marker'},{'o';'s';'>'})
set([h1;h2],{'MarkerFaceColor'},{'blue';'none';'red'});
title('Trimmed level flight by Gamma');
